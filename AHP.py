...
def save_response(respondent_name, A, B, W, criterias, alternatives):
    ...
    # Check if respondent already exists in session state
    existing_respondent = next((r for r in st.session_state.responses if r['respondent_name'] == respondent_name), None)
    
    if existing_respondent:
        # Update existing respondent's data
        existing_respondent.update(response_data)
    else:
        # Add new respondent to session state
        ...
        if cri and alt:
            ...
            if st.button("Hitung dan Simpan AHP"):
                if not respondent_name:
                    ...
                    # Save response only if AHP calculation is successful
